<section style="background-image: url(<?=get_template_directory_uri();?>/assets/images/banner_background.jpg);" class="banner_with_foreground">

   <img src="<?=get_template_directory_uri()?>/assets/images/banner_foreground.png" alt="" class="banner_foreground"> 

   <div class="cta_and_text">

        <p class="banner_text sub_title text">
            leve sua empresa para a nova realidade digital
        </p>

        <h1 class="banner_title title text">
            A sua persona virtual com alta performance
        </h1>

        <p class="banner_text sub_title text">
            Para marcas inovadoras: Brand Persona PYXYS + Ilustraria 3D é o primeiro combo do Brasil
            de construção de avatares 3D, com tecnologia de ponta em computação gráfica, combinada
            à mais completa Estratégia de Content, Growth e Performance Data-driven
        </p>


        <button class="banner_cta button">Quero saber mais</button>

        <div class="sponsors">
            <p class="banner_sponsors text">Um Produto:</p>

            <img src="<?= get_template_directory_uri();?>/assets/images/pyxys-logo-4x.png" alt="" class="sponsor_img">
            <img src="<?=get_template_directory_uri()?>/assets/images/logo-ilustraria3d-branco.png" alt="" class="sponsor_img">
        </div>

   </div>

</section>