<div class="navbar_container">


	<div class="navbar">
	
	    <img src="<?= get_template_directory_uri(); ?>/assets/images/logo.png" alt="nn" class="logo">

	    <ul class="menu">

	        <li class="menu_item"><a href="" class="menu_link">Home</a></li>
	        <li class="menu_item"><a href="" class="menu_link">Por Que Brand Persona</a></li>
	        <li class="menu_item"><a href="" class="menu_link">O Mercado</a></li>
	        <li class="menu_item"><a href="" class="menu_link">Vantagens</a></li>
	        <li class="menu_item"><a href="" class="menu_link">Como Funciona</a></li>
	        <li class="menu_item"><a href="" class="menu_link">O Combo</a></li>
	        <li class="menu_item"><a href="#contato" class="menu_link">Contato</a></li>

	    </ul>

	    <button onclick="toggleMenu()" id="hamburger" class="hamburger_icon">
	        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 50 50"
	        style="fill:#FFFFFF;" class="hamburger_icon">
	        <path d="M 5 8 A 2.0002 2.0002 0 1 0 5 12 L 45 12 A 2.0002 2.0002 0 1 0 45 8 L 5 8 z M 5 23 A 2.0002 2.0002 0 1 0 5 27 L 45 27 A 2.0002 2.0002 0 1 0 45 23 L 5 23 z M 5 38 A 2.0002 2.0002 0 1 0 5 42 L 45 42 A 2.0002 2.0002 0 1 0 45 38 L 5 38 z"></path>
	        </svg>
	    </button>
	</div>


    <div class="menu_dropdown hidden"> <!--.menu_dropdown-->

        <ul class="menu_mobile">

            <li class="menu_item"><a href="" class="menu_link">Home</a></li>
            <li class="menu_item"><a href="" class="menu_link">Por Que Brand Persona</a></li>
            <li class="menu_item"><a href="" class="menu_link">O Mercado</a></li>
            <li class="menu_item"><a href="" class="menu_link">Vantagens</a></li>
            <li class="menu_item"><a href="" class="menu_link">Como Funciona</a></li>
            <li class="menu_item"><a href="" class="menu_link">O Combo</a></li>
            <li class="menu_item"><a href="#contato" class="menu_link">Contato</a></li>

        </ul>

    </div>

</div>